...
festim     <- function(lambda, fixed.effects = TRUE, linear = FALSE, boot = 100) {
  ...
  # Optimal instrument
  # A better instrument
  OptI   <- qpeer.optimal.inst(model = Quant1, Glist = G, boot = boot, nthreads = 1)
  
  # OptI    <- qpeer.optimal.inst(model = Quant1, Glist = G, boot = boot, nthreads = 5)
  Quant2  <- qpeer(formula = y ~ X + GX, 
                   excluded.instruments = ~ OptI, 
                   Glist = Gnorm,
                   structural = TRUE,
                   tau = tau,
                   fixed.effects = fixed.effects)
  ...
  # A better instrument
  OptI     <- qpeer.optimal.inst(model = Quant1a, Glist = G, boot = boot, nthreads = 1)
  Quant1a  <- qpeer(formula = y ~ X + GX, 
                   excluded.instruments = ~ OptI, 
                   Glist = Gnorm,
                   structural = TRUE,
  ...
  # A better Instrument
  OptI     <- qpeer.optimal.inst(model = Quant1b, Glist = G, boot = boot, nthreads = 1)
  Quant1b  <- qpeer(formula = y ~ X + GX, 
                    excluded.instruments = ~ OptI, 
                    Glist = Gnorm,
                    structural = TRUE,
  ...
  valins   <-  qpeer.test(Quant1, Quant2, which = "wald")$test["p-value"]
  ...
  names(valins) <- paste0(ifelse(fixed.effects, "FE.", ""), c("Val.Boot", "Val05.Boot", "Val10.Boot"))
  ...
  c(LIM, Quant1, Quant2, Ces, valins, Etest, Quant1a, Quant1b)
}
...
Est11      <- do.call(cbind, mclapply(1:nsim, function(i) festim(lambda1, FALSE, FALSE, 100), mc.cores = 8))
cat("DGP A with fixed effects\n")
Est12      <- do.call(cbind, mclapply(1:nsim, function(i) festim(lambda1, TRUE, FALSE, 100), mc.cores = 8))

# Decreasing lambda
lambda2    <- c(0.2, 0.3, 0.2, 0.05, 0)
cat("DGP B without fixed effects\n")
Est21      <- do.call(cbind, mclapply(1:nsim, function(i) festim(lambda2, FALSE, FALSE, 100), mc.cores = 8))
cat("DGP B with fixed effects\n")
Est22      <- do.call(cbind, mclapply(1:nsim, function(i) festim(lambda2, TRUE, FALSE, 100), mc.cores = 8))

# Concave lambda
lambda3    <- c(0.2, 0, 0.275, 0.275, 0)
cat("DGP C without fixed effects\n")
Est31      <- do.call(cbind, mclapply(1:nsim, function(i) festim(lambda3, FALSE, FALSE, 100), mc.cores = 8))
cat("DGP C with fixed effects\n")
Est32      <- do.call(cbind, mclapply(1:nsim, function(i) festim(lambda3, TRUE, FALSE, 100), mc.cores = 8))

# Convex lambda
lambda4    <- c(0.2, 0.275, 0, 0, 0.275)
cat("DGP D without fixed effects\n")
Est41      <- do.call(cbind, mclapply(1:nsim, function(i) festim(lambda4, FALSE, FALSE, 100), mc.cores = 8))
cat("DGP D with fixed effects\n")
Est42      <- do.call(cbind, mclapply(1:nsim, function(i) festim(lambda4, TRUE, FALSE, 100), mc.cores = 8))

# Concave lambda
lambda5    <- c(0.2, -0.05, 0.35, 0.15, 0.1)
cat("DGP E without fixed effects\n")
Est51      <- do.call(cbind, mclapply(1:nsim, function(i) festim(lambda5, FALSE, FALSE, 100), mc.cores = 8))
cat("DGP E with fixed effects\n")
Est52      <- do.call(cbind, mclapply(1:nsim, function(i) festim(lambda5, TRUE, FALSE, 100), mc.cores = 8))

# Constant lambda with data simulated using the standard LIM model
lambda6    <- 0.55
cat("DGP F without fixed effects\n")
Est61      <- do.call(cbind, mclapply(1:nsim, function(i) festim(lambda6, FALSE, TRUE, 100), mc.cores = 8))
cat("DGP F with fixed effects\n")
Est62      <- do.call(cbind, mclapply(1:nsim, function(i) festim(lambda6, TRUE, TRUE, 100), mc.cores = 8))

save(Est11, Est12, Est21, Est22, Est31, Est32, Est41, Est42, Est51, Est52, Est61, Est62,
     file = paste0(OutResPath, "/Simulations.Boot.Rda"))
...
load(paste0(OutResPath, "/Simulations.Boot.Rda"))
...
T5.1 <- Est %>% select(all_of(c(paste0("FE.Q1.y_q", 1:4), "FE.Q1.y_q(conformity)",
                              paste0("FE.Q2.y_q", 1:4), "FE.Q2.y_q(conformity)",
...
T5.1 <- cbind(T5.1[,c("FE.Q1.y_q1", "FE.Q1.y_q2", "FE.Q1.y_q3", "FE.Q1.y_q4", "FE.Q1.y_q(conformity)")], "V1" = NA,
              T5.1[,c("FE.Q2.y_q1", "FE.Q2.y_q2", "FE.Q2.y_q3", "FE.Q2.y_q4", "FE.Q2.y_q(conformity)")], "V2" = NA,
...
T5.2 <- Est %>% select(all_of(c("FE.ET05.ntau=3.4", "FE.ET10.ntau=3.4", 
                                ...
                                "FE.Q1.KPstat", "FE.Q2.KPstat" # KP stat
                                ))) %>% # Validity of Z2
  mutate(across(.cols = -c(FE.Q1.KPstat, FE.Q2.KPstat),
...
T5.2 <- cbind(T5.2[,c("FE.ET05.ntau=3.4", "FE.ET10.ntau=3.4", 
              ...
              T5.2[,c("FE.Q1.KPstat", "FE.Q2.KPstat")])
# write
writeData(wb, "Table 5.2", T5.2, keepNA = TRUE, na.string = "", startRow = 1, startCol = 1)
# first row
fr    <- c("3vs4, pval = 5%", "3vs4, pval = 10%", "4vs5, pval = 5%", "4vs5, pval = 10%", "", "KP Z", "KP Best Inst")
for (i in 1:length(fr)) {
...
# Save the workbook
saveWorkbook(wb, paste0(OutResPath, "/simulations.Boot.xlsx"), overwrite = TRUE)
