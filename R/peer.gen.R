#' @rdname qpeer
#' @export
genpeer <- function(formula, excluded.instruments, endogenous.variables, Glist, data, gmm.weight = "IV", 
  ...
  Kx         <- ncol(X)
  if (structural) {
    if (length(c(fcheckrank(X = cbind(endo, X)[nIs + 1,], tol = tol))) != (Kendo + Kx)) stop("The design matrix is not full rank.")
  } else {
    if (length(c(fcheckrank(X = cbind(endo, X), tol = tol))) != (Kendo + Kx)) stop("The design matrix is not full rank.")
  }
  ...
  if (structural) {
    ...
    # Estimation
    GMMe     <- fgmm_struc(y = y, X = X, qy = endo, ins = ins, W1 = diag(Kx1), W2 = diag(Kins + 1), idX1 = idX1, 
                           idX2 = idX2, Kx1 = Kx1, Kx2 = Kx2, igroup = igr, nIs = nIs, Is = Is, ngroup = M, 
                           Kins = Kins, Kx = Kx, ntau = Kendo, Kest = Kest, n = n, HAC = HACnum, iv = iv)
    if (gmm.weight == "optimal" & HACnum != 0) {
      GMMe   <- fgmm_struc(y = y, X = X, qy = endo, ins = ins, W1 = solve(GMMe$VF1), W2 = solve(GMMe$VF2), idX1 = idX1, 
                           idX2 = idX2, Kx1 = Kx1, Kx2 = Kx2, igroup = igr, nIs = nIs, Is = Is, ngroup = M, Kins = Kins, 
                           Kx = Kx, ntau = Kendo, Kest = Kest, n = n, HAC = HACnum, iv = FALSE)
    }
    Vpa        <- fStructParam(param = c(GMMe$beta, GMMe$lambda), covp = GMMe$Vpa, idX1 = idX1, idX2 = idX2, 
                               ntau = Kendo, Kx = Kx, Kx1 = Kx1, Kx2 = Kx2)
    ...
    GMMe     <- fgmm_red(y = y, V = V, ins = ins, W = diag(Kins), igroup = igr, ngroup = M, 
                         Kx = Kx, Kins = Kins, ntau = Kendo, Kest = Kest, n = n, HAC = HACnum, iv = iv)
    if (gmm.weight == "optimal" & HACnum != 0) {
      GMMe   <- fgmm_red(y = y, V = V, ins = ins, W = solve(GMMe$VZe), igroup = igr, ngroup = M, Kx = Kx, 
                         Kins = Kins, ntau = Kendo, Kest = Kest, n = n, HAC = HACnum, iv = FALSE)
    }
  }
  fv         <- c(GMMe$yhat)
  res        <- y - fv
  ...
  out       <- list(model.info  = list(n = n, ngroup = M, nvec = nvec, structural = structural, formula = formula, 
                                      ...
                                      gmm.weight = gmm.weight, fixed.effects = fixed.effects, idX1 = idX1 + 1, idX2 = idX2 + 1, HAC = HAC),
                    gmm         = GMMe,
