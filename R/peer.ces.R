...
#' 
#' ## Reduced-form model
#' rest <- cespeer(formula = y ~ X, instrument = ~ z, Glist = Gnorm, fixed.effects = "yes", 
#'                 radius = 5)
#' summary(rest)
#' 
#' ## Structural model
#' sest <- cespeer(formula = y ~ X, instrument = ~ z, Glist = Gnorm, fixed.effects = "yes", 
#'                 radius = 5, structural = TRUE)
#' summary(sest)
#' @export
...
cespeer <- function(formula, instrument, Glist, structural = FALSE, fixed.effects = FALSE,
  ...
  out         <- list(model.info   = list(n = n_st, ngroup = M, nvec = nvec, structural = structural, formula = formula, 
                                         instrument = instrument, fixed.effects = fixed.effects, HAC = HAC, 
                                         set.rho = set.rho),
                      gmm          = Est,
                      first.search = fes)
  class(out)  <- "cespeer"
...
#'     \item{gmm}{A list of GMM estimation results, including parameter estimates, the covariance matrix, and related statistics.}
#' @export
summary.cespeer <- function(object, ...) {
